<html>
    <head>
        <title>Main POS</title>
        <link rel="stylesheet" href="styles.css">
        <link href='https://unpkg.com/boxicons@2.1.2/css/boxicons.min.css' rel='stylesheet'>
        <script src="https://unpkg.com/boxicons@2.1.2/dist/boxicons.js"></script>
    </head>
    <body>
        <div class="wrapper">
            <div class="sidebar">
                <h2>Sandra's Store</span></h2>
                <ul>
                    <li><a href="#"><i class='bx bx-home-circle' ></i>Dashboard</a></li>
                    <li><a href="#"><i class='bx bxs-user-plus'></i>User Registration</a></li>
                    <li><a href="#"><i class='bx bx-food-menu' ></i></i>Inventory</a></li>
                    <li><a href="#"><i class='bx bx-purchase-tag-alt'></i></i>Purchase Order</a></li>
                    <li><a href="#"><i class='bx bx-scan'></i></i>POS</a></li>
                    <li><a href="#"><i class='bx bx-notepad'></i>Online Orders</a></li>
                    <li><a href="#"><i class='bx bx-time' ></i></i>Order History</a></li>
                    <li><a href="#"><i class='bx bxs-report'></i>Sales Report</a></li>
                    <li><a href="#"><i class='bx bxs-chat' ></i>Messages</a></li>
                    <li><a href="#"><i class='bx bxs-user-account'></i>Account Management</a></li>
                </ul> 
            </div>
            <!--This section is for the main content of the current selected navigation-->
            <div class="main_content">
                <!--This section is for the to be shipped today widget-->
                <div>
                </div>
            </div>
        </div>
    </body>
</html>